...
bool is_available(ListaFiltros* catalogo, int indice) {
  for (int i = 0; i < indice; i++, catalogo = catalogo->prox)
    ;
  return catalogo->filtro->max_instancias - catalogo->filtro->em_processamento >
         0;
}
...
bool processa_pedido(CatalogoFiltros* catalogo, Request* req) {
  ...
  if (req->request_type == TRANSFORM) {
    // TO FIX
    for (int i = 0, j = 0; i < catalogo->size && j < req->number_filters; i++) {
      if (req->requested_filters[i] > 0) {
        if (!is_available(lista_filtros, i)) {
          return false;
        }
        j++;
      }
...
int main(int argc, char* argv[]) {
  ...
  while (!stop && (bytes_read = read(
                       fd_leitura, new_request, sizeof(struct request))) > 0) {
