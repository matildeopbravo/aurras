...
/* return -1 if filter does not exist (otherwise return the filter_index for Request)*/
...
/* return the argv postion of the filter that does not exist (otherwise return -1) */
...
prs_pointer transform(int argc, char** argv) {
    ...
    if ((invalid_filter = parser_filters(&request, argv, argc)) != -1){
        printf("Filter \"%s\" does not exist. Please insert a valid filter\n",argv[invalid_filter]);
        return NULL;
    }
